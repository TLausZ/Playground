...
Two scenes, switched with the space bar: squares (hinged at a corner) and circles (hinged at the tangent point where two shapes touch). A static area fill underneath traces each shape's full rotation as a disc, so overlapping shapes darken toward the centre and form a bullseye.
